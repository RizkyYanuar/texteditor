...
void backspace(addressBar FirstBar, addressBar *CurrentBar, addressKol *Cursor, int *CursorX, int *CursorY, SelectPoint SelStart, SelectPoint SelEnd, int *Selecting) {
		if (*Selecting == 1) {
			...
			if (start->prev == Nil && end->next == Nil) { // jika select huruf dari awal sampai akhir
				...
				(*CurrentBar)->kol = Nil;
				(*CurrentBar)->longBar = 0;
				(*CurrentBar)->tail = Nil;
				*Cursor = Nil;
				*CursorX = 5;
			} else if (start->prev == Nil && end->next != Nil) { // jika select dari awal sampai tengah
				int jumlahNodeDihapus = 0;
				while (start != stop) {
					temp = start;
					start = start->next;
					free(temp);
				...
				(*CurrentBar)->kol = start;
				(*CurrentBar)->kol->prev = Nil;
				(*CurrentBar)->longBar -= jumlahNodeDihapus;
				*Cursor = (*CurrentBar)->kol;
				*CursorX = 5;
			} else if (start->prev != Nil && end->next == Nil) { // jika select dari tengah sampai akhir
				int jumlahNodeDihapus = 0;
				addressKol nodeSebelumSelStart = start->prev;
				nodeSebelumSelStart->next = Nil;
				while (start != stop) {
					temp = start;
					start = start->next;
					free(temp);
				...
				(*CurrentBar)->tail = nodeSebelumSelStart;
				(*CurrentBar)->longBar -= jumlahNodeDihapus;
				*Cursor = (*CurrentBar)->tail;
				if (SelStart.x <= SelEnd.x) { // jika arah selection dari kiri ke kanan
					*CursorX = PCX(*CursorX, -1 * jumlahNodeDihapus);
				}
			} else if (start->prev != Nil && end->next != Nil) { // jika select dari tengah ke tengah
				addressKol nodeSebelumSelStart = start->prev;
				int jumlahNodeDihapus = 0;
				...
				(*CurrentBar)->longBar -= jumlahNodeDihapus;
				*Cursor = nodeSebelumSelStart;
				if (SelStart.x <= SelEnd.x) { // jika arah selection kiri ke kanan
					*CursorX = PCX(*CursorX, -1 * jumlahNodeDihapus);
				}
			}
			*Selecting = 0;
